...
%
function plotLineageTreeActivity(trees, annotation, colorGroup, fillGroup, activeNeuronMat)
treeSpacing = 3;
nodeSpacing = 2;
...
for i = 1:size(nodes2Fill, 1)
    ...
    plot(xSquare, [endTime, endTime+size(activeNeuronMat, 2), endTime+size(activeNeuronMat, 2), endTime], 'k');
    for j = 1:size(activeNeuronMat, 2)
        if activeNeuronMat(i, j)
            fill(xSquare, [endTime+j-1, endTime+j, endTime+j, endTime+j-1], colorGroup(i), 'LineStyle', 'None');
        end
    end
    if fillGroup(i) == 0
        text(xNode, endTime+size(activeNeuronMat, 2)+50, '*', 'FontSize', 40, 'HorizontalAlignment', 'center');
    end
end
