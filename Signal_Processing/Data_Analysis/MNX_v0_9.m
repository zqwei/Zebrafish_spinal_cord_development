...
function MNX_v0_9(nFile)
    ...
    for period = 1:numel(timePoints)
        ...
        subplot(3, 2, 2)
        hold on
        plot(x(activeTag & mnx), y(activeTag & mnx), 'ok', 'MarkerFaceColor','k', 'linewidth', linew);
        plot(x(activeTag & ~mnx), y(activeTag & ~mnx), 'sk', 'MarkerFaceColor','k', 'linewidth', linew);
        if size(LMat,2) >= 1
            for nFactor = 1:size(LMat, 2)
...
% %                 end
        ...
        plot(x(~activeTag & ~mnx), y(~activeTag & ~mnx), 'sk', 'linewidth', linew, 'MarkerFaceColor', 'w');
        for i = 1:ceil(max(x))
            plot([i, i], [-2, 2], '--k');
        end
        xlim([0 ceil(max(x))+1]);
        ...
        subplot(3, 2, 6)
        hold on
        plot(x(activeTag & side==1 & mnx), z(activeTag & side==1 & mnx), 'ok', 'MarkerFaceColor','k', 'linewidth', linew);
        plot(x(activeTag & side==1 & ~mnx), z(activeTag & side==1 & ~mnx), 'sk', 'MarkerFaceColor','k', 'linewidth', linew);
        if size(LMat,2) >= 1
            for nFactor = 1:size(LMat, 2)
                neuronFactor = LMat(:, nFactor)>0;
...
% %                 end
        ...
        plot(x(~activeTag & side==1 & mnx), z(~activeTag & side==1 & mnx), 'ok', 'linewidth', linew, 'MarkerFaceColor', 'w');
        plot(x(~activeTag & side==1 & ~mnx), z(~activeTag & side==1 & ~mnx), 'sk', 'linewidth', linew, 'MarkerFaceColor', 'w');
        for i = 1:ceil(max(x))
            plot([i, i], [0, 2], '--k');
        end
        xlim([0 ceil(max(x))+1]);
        ...
        subplot(3, 2, 4)
        hold on
        plot(x(activeTag & side==2 & mnx), z(activeTag & side==2 & mnx), 'ok', 'MarkerFaceColor','k', 'linewidth', linew);
        plot(x(activeTag & side==2 & ~mnx), z(activeTag & side==2 & ~mnx), 'sk', 'MarkerFaceColor','k', 'linewidth', linew);
        if size(LMat,2) >= 1
            for nFactor = 1:size(LMat, 2)
                neuronFactor = LMat(:, nFactor)>0;
...
% %                 end
        ...
        plot(x(~activeTag & side==2 & mnx), z(~activeTag & side==2 & mnx), 'ok', 'linewidth', linew, 'MarkerFaceColor', 'w');
        plot(x(~activeTag & side==2 & ~mnx), z(~activeTag & side==2 & ~mnx), 'sk', 'linewidth', linew, 'MarkerFaceColor', 'w');
        for i = 1:ceil(max(x))
            plot([i, i], [0, 2], '--k');
        end
        xlim([0 ceil(max(x))+1]);
