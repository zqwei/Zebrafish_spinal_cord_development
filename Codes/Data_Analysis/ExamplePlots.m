...
load([tempDatDir, fileName, '.mat'], 'sideSplitter', 'side', 'tracks', 'timePoints', 'activeNeuronMat','mnx', 'timeStep'); 
...
figure;
plot((1:timeStep)/4, slicedDFF(:, nNeuron), '-k', (1:timeStep)/4, DFFEst, '-r')
ylabel('z-DF/F')
xlabel('Time (s)')
xlim([0 300])
box off
...
cla reset
hold on
xtracks   = squeeze(mean(tracks(:, timePoints(nTime)+(1:timeStep), 1), 2));
...
figure;
hold on
plot((1:timeStep)/4, bsxfun(@plus, slicedDFF(:, sum(lambda,2)==0), find(sum(lambda,2)==0)'*5), '-', 'color',[0.5 0.5 0.5])
plot((1:timeStep)/4, bsxfun(@plus, slicedDFF(:, lambda(:,1)~=0), find(lambda(:,1)~=0)'*5), '-b')
plot((1:timeStep)/4, bsxfun(@plus, slicedDFF(:, lambda(:,2)~=0), find(lambda(:,2)~=0)'*5), '-r')
% ylabel('z-DF/F')
% xlabel('Time (s)')
axis off
box off
...
figure;
hold on
plot((1:timeStep)/4, bsxfun(@plus, DFF(1, :), 1*5), '-b')
plot((1:timeStep)/4, bsxfun(@plus, DFF(2, :), 2*5), '-r')
axis off
hold off
