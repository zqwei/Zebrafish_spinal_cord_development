...
function FACluster_v1_5(nFile)           
    ...
    for nDat  = 6%1:length(saveFileNames)
...
%         subplot(3, 1, 1)
%         hold on
%         plot((1:numTime)/60, corrStd, '-k', 'linewid', 2)
%         hold off
%         xlabel('Time (hour)')
%         ylabel('Mean corr.')
%         xlim([0 numTime/60])
...
%         subplot(3, 1, 2)
        delayStd(isnan(delayStd) & randomMean==0) = 0;
        hold on
        plot((1:numTime)/60, delayStd, 'o', 'color', [     0    0.4470    0.7410])
        yy = smooth((1:numTime)/60, delayStd, 15);
        plot((1:numTime)/60, yy, '--', 'color', [     0    0.4470    0.7410])
        hold off
        xlabel('Time (hour)')
        ylabel('Std delay (s)')
        xlim([0 numTime/60])
        ...
        disp(find(yy==0 & randomMean==0, 1, 'first')/60)
