...
%  revision:
% 
%  anti-image correlation matrix A = D invR D
%                                D = inv(sqrt(diag(R^-1)))
%
%
%  References:
...
R        = corr(X);
invR     = eye(size(R))/R;
D        = diag(1./sqrt(diag(invR)));
A        = D * invR * D;
sumR     = sum((R-diag(diag(R))).^2); % sum_i<>j r_ij^2
sumA     = sum((A-diag(diag(A))).^2); % sum_i<>j a_ij^2

MSA      = sumR ./ (sumR+sumA); % KMO_i
KMO      = sum(sumR) / sum(sumR + sumA);
