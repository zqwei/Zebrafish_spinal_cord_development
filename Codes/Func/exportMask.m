...
function exportMask(inputFileName, outputFilePath, coordinates, colorTable)
    ...
    % rgbStack = repmat( (stack - min(stack(:)))/(max(stack(:)) - min(stack(:))), [1,1,1,3]);
    ...
    %     for c = 1:3
    %         rgbStack(coordinates(i, 2)-radius:coordinates(i, 2)+radius, coordinates(i, 1)-radius:coordinates(i, 1)+radius, coordinates(i, 3)-radius/ratio:coordinates(i, 3)+radius/ratio, c) = colorTable(i, c);
    %     end
        for j = 1:length(cmx)
