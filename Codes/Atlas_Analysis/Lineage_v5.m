...
%         idxTrue(side==side(sibling(i, 2)), side==side(sibling(i, 1))) = true;
        ...
        EVsim(i, 5) = numel(EVcontrolVector); % number of pairs to control
        EVsim(i, 6) = side(siblings(i, 1)) == side(siblings(i, 2)); %ipsilateral pair(1), or contraleteral pair(0)
...
ipSibList = EVsimAll(EVsimAll(:, 6)==1 & ~isnan(EVsimAll(:, 1)), :);
ipCtrlVec = EVctrlAll(EVsimAll(:, 6)==1 & ~isnan(EVsimAll(:, 1)));
figure,     hold on
