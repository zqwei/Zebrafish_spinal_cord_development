...
useTagAfter = 1; %0: use all neurons that are active before, 1: use neurons that are active before & after
tagExt = [];
...
for i = 1:numel(fishList)
    ...
    for nExp = 1:2
        ...
        if nExp == 1
            activeTag = activeNeuronMat;
        else
           load ([tempDatDir, fileName, '.mat'], 'activeTagBefore', 'activeTagAfter');
           activeTag = activeTagBefore;
           if useTagAfter
               activeTag = activeTag & activeTagAfter;
           end
        end
