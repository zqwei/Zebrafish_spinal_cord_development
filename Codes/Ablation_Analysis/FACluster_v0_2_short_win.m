...
function FACluster_v0_2_short_win(nFile)
    ...
    for nTime         = 1:numTime
        if sum(~isnan(EVLONOMat(nTime, :))) > 0
            ...
            if maxEV>0
                maxId = find(EVLONOMat(nTime, :) == maxEV, 1, 'first');
                evMat = squeeze(EVLONO(nTime, :, :));
                for nDim = 1:maxId
                    h = ttest2(evMat(nDim,:), evMat(maxId,:)*0.9, 'tail', 'left');
                    if ~h
                        maxDim = nDim;
                        break;
                    end
