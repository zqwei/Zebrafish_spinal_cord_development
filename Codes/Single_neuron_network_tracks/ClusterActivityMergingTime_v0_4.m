...
% Plot Location vs size of cluster
%
% -------------------------------------------------------------------------
% 
% Ziqiang Wei
...
% function ClusterActivityMergingTime_v0_4(nFile)
addpath('../Func');
setDir;    
plot_Mat      = [];
    ...
    for nFactor   = 1:numFactor
        ...
        if max(timeInd) - min(timeInd) > 5
            ...
            if max(timeInd) < numTime
                ...
                if max_value > 0
%                     disp([mean(new_x(LMat)), mean(new_x(sum(globalLMat,2)>0)), sum(LMat), sum(sum(globalLMat,2)>0)])
                    plot_Mat = [plot_Mat; max(timeInd), sum(LMat), sum(sum(globalLMat,2)>0)];
                end
            end
        end
    end   
...
figure;
scatter(plot_Mat(:, 3), plot_Mat(:, 2), [], plot_Mat(:, 1), 'filled')
