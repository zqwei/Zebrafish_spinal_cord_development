function Plot_Traces_Map_black(nFile)
...
frameW = 2000;
frameH = 1000;
fig = figure('units', 'pixels', 'position', [0 0 , frameW, frameH], 'Color', 'black');
...
%                 inside = inpoly([x, y], CHPoints);
%                 [v, c] = voronoin([x(inside&~activeTag), y(inside&~activeTag); CHPoints; x(inside&activeTag), y(inside&activeTag)]);
%                 for i = 1: sum(inside&~activeTag)
%                     patch(v(c{i}, 1), v(c{i}, 2), 'w','edgecolor', 'none');
%                 end
            else
    ...
    if size(LMat,2) >= 1
        for nFactor = 1:size(LMat, 2)
            ...
            if length(unique(side(neuronFactor)))==1 && unique(side(neuronFactor))==1
                ...
                patch(CHPoints(:,1), CHPoints(:,2), mColor(factorIndex(nFactor), :), 'facealpha', 0.6, 'edgecolor', 'none');
%                 inside = inpoly([x, z], CHPoints);
%                 [v, c] = voronoin([x(side==1&inside&~activeTag), z(side==1&inside&~activeTag); CHPoints; x(side==1&inside&activeTag), z(side==1&inside&activeTag)]);
%                 for i = 1: sum(side==1&inside&~activeTag)
%                     patch(v(c{i}, 1), v(c{i}, 2), 'k','edgecolor', 'none');
%                 end
            else
                if sum(side(neuronFactor)==1) == 1 || sum(side(neuronFactor)==2) == 1
    ...
    if size(LMat,2) >= 1
        for nFactor = 1:size(LMat, 2)
            ...
            if length(unique(side(neuronFactor)))==1 && unique(side(neuronFactor))==2
                ...
                patch(CHPoints(:,1), CHPoints(:,2), mColor(factorIndex(nFactor), :), 'facealpha', 0.6, 'edgecolor', 'none');
%                 inside = inpoly([x, z], CHPoints);
%                 [v, c] = voronoin([x(side==2&inside&~activeTag), z(side==2&inside&~activeTag); CHPoints; x(side==2&inside&activeTag), z(side==2&inside&activeTag)]);
%                 for i = 1: sum(side==2&inside&~activeTag)
%                     patch(v(c{i}, 1), v(c{i}, 2), 'k','edgecolor', 'none');
%                 end
            else
                if sum(side(neuronFactor)==1) == 1 || sum(side(neuronFactor)==2) == 1
