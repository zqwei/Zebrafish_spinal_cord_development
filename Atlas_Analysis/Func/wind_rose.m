...
for i=1:length(Ay)-1
  if n(i)
    ...
    for j=1:length(Ag)-1
      ...
      if E(i,j)>0, handles{end+1}=fill(x,y,cor{jcor}, 'EdgeColor', lineColors); set(handles{end}, 'facealpha', .5); end
      r1=r2;
    end
  end
end
...
elseif legType==2 % separated boxes.
  for j=1:length(Ag)-1
    ...
    y1=y0+h;
    handles{end+1}=fill([x0 x1 x1 x0],[y0+dy y0+dy y1 y1],cor{j}, 'EdgeColor', lineColors);
